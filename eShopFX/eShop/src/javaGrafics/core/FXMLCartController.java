 ...
 * @author avatar
 */
public class FXMLCartController implements Initializable {
    ...
    /**
     * 
     * Método que caso o cep esteja preenchido abre a cena de carrinho com frete 
     * calculado, caso contrário, mostra uma mensagem com um alerta.
     * 
     * @param event
     * @throws IOException 
     */
    @FXML
    private void buttonFreightAction(ActionEvent event) throws IOException {
        if (!zipCode.getText().matches("[0-9]{2}.[0-9]{3}-[0-9]{3}")) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("CEP inválido");
                alert.setHeaderText("CEP inválido!");
                alert.setContentText("Por favor, digite um CEP válido!");
                alert.showAndWait();
        } else {
           Parent CartFreight_page_parent = FXMLLoader.load(getClass().getResource("FXMLCartFreight.fxml"));
           
           Scene CartFreight_page_scene = new Scene(CartFreight_page_parent);
           
           Stage app_stage = (Stage) ((Node) event.getSource()).getScene().getWindow();

           app_stage.hide();
           app_stage.setTitle("Carrinho de compras");
           app_stage.setScene(CartFreight_page_scene);
           app_stage.show();   
        }
    }
