package cosmapek.analyzer.core;
...
public class Analyzer implements IInterfaceTags {
    ...
    private void analyzeSensors() {
        ISensors sensors = sensor.getSensors();//give me the cosmapek.sensors in run time

        failedFeatures.clear();
        for (ISensor sensor : sensors) {
            if (sensor.isActivated()) {
                failedFeatures.add(sensor.getFeatureName());//add the failed cosmapek.sensors
                //TO TEST
                long time= System.currentTimeMillis();
                android.util.Log.i("Eshop", "Analyzer_SensorFlagActivate: " + time);
            }
        }
    }
