...
public class NodeReader {
    ...
    public final static String[] textualSymbols =
        new String[] {"iff", "implies", "or", "and", "not"};
     ...
     *
     * @param string  constraint (without brackets) to convert
     * @param symbols array containing strings for: iff, implies, or, and, not
     * @param list    list of substituted bracket expressions
     * @return
     */
        ...
        if (!error) {
            ...
            for (int i = 0; i < symbols.length; i++) {
                while (string.contains(" " + symbols[i] + " ")) {
                    ...
                    Node node1 = null;
                    if (i != 4)
                        node1 = checkExpression(string.substring(0, index), list);
                    Node node2 = checkExpression(
                        string.substring(index + symbols[i].length(), string.length()), list);
        ...
        if (!constraint.trim().isEmpty()) {
            ...
            while (constraint.contains("\"")) {
                ...
                if (indStart - 1 > 0 && constraint.charAt(indStart - 1) != ' '
                    ...
                    || indEnd + 1 < constraint.length() && constraint.charAt(indEnd + 1) != ' '
                ...
                constraint = constraint.substring(0, indStart) + (counter + 100000) + constraint
                    .substring(indEnd + 1, constraint.length());
                counter++;
            }
            ...
            while (constraint.contains(")")) {
                ...
                // inner bracket found -> substitution to list
                list.add(constraint.substring(indStart + 1, indEnd));
                constraint = constraint.substring(0, indStart) + counter + constraint
                    .substring(indEnd + 1, constraint.length());
                counter++;
            }
    ...
    private Node getNode(String string) {
        ...
        while (string.contains("\"")) {
            ...
            string = string.substring(0, indStart) + (counter + 100000) + string
                .substring(indEnd + 1, string.length());
            counter++;
        }
        ...
        while (string.contains(")")) {
            ...
            // inner bracket found -> substitution to list
            list.add(string.substring(indStart + 1, indEnd));
            string = string.substring(0, indStart) + counter + string
                .substring(indEnd + 1, string.length());
            counter++;
        }
    ...
    private Node stringToNodeRec(String string, List<String> list) {
        ...
        for (int i = 0; i < symbols.length; i++) {
            while (string.contains(" " + symbols[i] + " ")) {
                ...
                Node node2 =
                    stringToNodeRec(string.substring(index + symbols[i].length(), string.length()),
                        list);
