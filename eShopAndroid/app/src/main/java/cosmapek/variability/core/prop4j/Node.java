...
public abstract class Node {
    ...
    @SuppressWarnings("unchecked") public void setChildren(Object... newChildren) {
    ...
    @SuppressWarnings("unchecked") public Node toCNF() {
        Node node = this;
    ...
    @SuppressWarnings("unchecked") public Node toCNFprintln() {
        Node node = this;
     ...
     * cosmapek.connectors, e.g. And, are given as a parameter.
     *
     * @param symbols the symbols for logical cosmapek.connectors
    ...
    //... = is call varargs. Is like an clean array.
    ...
    //Si yo no soy el literal con el nombre lo mando a buscar a mis hijos
    public List<Node> replaceFeature(IFeature feature, IFeature replaceWithFeature,
        List<Node> list) {
    ...
    protected void fuseWithSimilarChildren() {
        ...
        for (Node child : children)
            if (getClass().isInstance(child))
                count += child.children.length - 1;//voy a agregar todos mis hijos
        Node[] newChildren = new Node[count];// arreglo con nuevo contador
        int i = 0;
        for (Node child : children) {
            if (getClass().isInstance(child))
