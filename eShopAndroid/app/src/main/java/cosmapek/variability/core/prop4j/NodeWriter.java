...
public class NodeWriter {
    ...
    public final static String[] textualSymbols =
        new String[] {"not ", "and", "or", "implies", "iff", ", ", "choose", "atleast", "atmost"};

    /**
     * short textual representation
     */
    public final static String[] shortSymbols =
        new String[] {"-", " & ", " | ", " => ", " <=> ", ", ", "choose", "atleast", "atmost"};
    ...
    public final static String[] javaSymbols =
        new String[] {"!", " && ", " || ", noSymbol, " == ", noSymbol, noSymbol, noSymbol,
            noSymbol};
     ...
     * @return the textual representation
     */
    protected static String nodeToString(Node node, String[] symbols, boolean optionalBrackets,
        Class<? extends Node> parent) {
        return nodeToString(node, symbols, optionalBrackets, false, parent);
    }

    protected static String nodeToString(Node node, String[] symbols, boolean optionalBrackets,
        boolean addQuotationMarks) {
     ...
     */
    protected static String nodeToString(Node node, String[] symbols, boolean optionalBrackets,
        boolean addQuotationMarks, Class<? extends Node> parent) {
        ...
        if (node instanceof Not)
            return symbols[0] + " " + nodeToString(node.getChildren()[0], symbols, optionalBrackets,
                addQuotationMarks, node.getClass());
     ...
     * @return the textual representation
     */
    protected static String multipleNodeToString(Node node, String[] symbols,
        boolean optionalBrackets, Class<? extends Node> parent, boolean addQuotationMarks) {
        ...
        for (Node child : children) {
            s.append(separator);
            s.append(
                nodeToString(child, symbols, optionalBrackets, addQuotationMarks, node.getClass()));
        }
    ...
    protected static int order(Class<? extends Node> nodeClass) {
        if (nodeClass == null)
            return 0;
        if (nodeClass.equals(AtMost.class) || nodeClass.equals(AtLeast.class) || nodeClass
            .equals(Choose.class))
            return 1;
